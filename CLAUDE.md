...
- **`facturas_ligeras` NUNCA debe tocar el contenido de los adjuntos.** La vista arma el cascarón con `CASE WHEN pdf_x IS NOT NULL THEN jsonb_build_object('_enNube',true,'name','<etiqueta>') END`, **no** con `_cascaron(pdf_x, ...)`. Motivo (11 sep 2026): pasarle la columna a `_cascaron` obliga a Postgres a *destoastear* el base64 completo de cada adjunto para sacarle el `name`. Eran **2,635 ms** para 199 renglones contra el **statement_timeout de 3 s del rol `anon`** (ISA-OS entra con la llave anon), así que **el 61 % de las bajadas moría con 500 / `57014`** — en la laptop de Isaac 51 de 83. La app se quedaba con la copia vieja y seguía diciendo "Sincronizado": por eso Adrián subía una factura y en unos equipos no aparecía. Con el cascarón por presencia la misma consulta tarda **1.1 ms**. Si algún día necesitas el nombre real del archivo en la lista, **no lo saques de la columna** — guárdalo en una columna aparte; leer el jsonb de vuelta revive el bug. El nombre real igual llega al abrirlo: `verAdjuntoFact` hidrata primero y luego arma el nombre con `nombreDocFactura`.
- **`cloudFetchAll` nunca debe devolver una lista parcial.** Quien la llama reemplaza el arreglo completo (`contactos`, `bomRules` y `modulos` se asignan directo con `=`), así que una página perdida borraba registros buenos de la copia local. Desde v2026.09.10.02 reintenta 3 veces con espera, devuelve `null` si falla, y anota el fallo con `_avisarBajadaFallida` (Diagnóstico + aviso en pantalla, máx. 1 cada 5 min por tabla).
- **Paginar `facturas_ligeras` con `OFFSET` EMPEORA las cosas** — está medido: la tabla completa eran 2,635 ms y la página 3 de 50 renglones 4,023 ms, porque el servidor recalcula también las filas que va a tirar. Si vuelve a ponerse lenta, el arreglo va en la vista, no en el tamaño de página.
